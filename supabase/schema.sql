...
create table if not exists public.tickets (
  ...
  status text not null check (status in ('Чернетка', 'Нове повернення', 'На перевірку', 'Повернення коштів', 'Повернення здійснено ✅', 'На доопрацювання', 'Завершено без повернення', 'Відхилено ❌')),
  order_number text,
  warehouse_fop text,
